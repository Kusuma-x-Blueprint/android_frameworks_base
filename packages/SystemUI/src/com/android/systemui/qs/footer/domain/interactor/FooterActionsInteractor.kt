...
interface FooterActionsInteractor {
    ...
    /** Show the LineageParts */
    fun showLineageParts(expandable: Expandable): Boolean
...
constructor(
    ...
    override fun showLineageParts(expandable: Expandable): Boolean {
        if (!deviceProvisionedController.isCurrentUserSetup) {
            // If user isn't setup just unlock the device and dump them back at SUW.
            activityStarter.postQSRunnableDismissingKeyguard {}
            return false
        ...
        metricsLogger.action(MetricsProto.MetricsEvent.ACTION_QS_EXPANDED_SETTINGS_LAUNCH)
        activityStarter.startActivity(
            Intent("org.lineageos.lineageparts.STATUS_BAR_SETTINGS"),
            true /* dismissShade */,
            expandable.activityLaunchController(
                InteractionJankMonitor.CUJ_SHADE_APP_LAUNCH_FROM_SETTINGS_BUTTON
            ),
